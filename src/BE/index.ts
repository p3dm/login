...
import { signUp, signIn, signOut, getSessionUser, parseCookies, confirmEmail, resendConfirmation, forgotPassword, changePassword, signInWithGoogle, handleOAuthCallback } from "./auth";

const PORT = process.env.PORT || 3000;

// ─── CORS ─────────────────────────────────────────────────────────────────────
// Danh sách origin được phép — thêm vào ALLOWED_ORIGINS trong .env
// VD: ALLOWED_ORIGINS=http://localhost:5173,https://yourdomain.com
...
const server = http.createServer(async (req, res) => {
  ...
  // ── CORS: set headers cho mọi response ──────────────────────────────────────
  setCors(req, res);

  // ── Preflight OPTIONS — trình duyệt gửi trước mọi cross-origin request ──────
  if (method === "OPTIONS") {
    res.writeHead(204);
    res.end();
    return;
  ...
  try {
    ...
    if (method === "GET" && url === "/auth/google") {
      const { url: googleUrl } = await signInWithGoogle();
      res.writeHead(302, { Location: googleUrl });
      res.end();
      return;
    }

    // GET /auth/callback — Google redirect về đây kèm ?code=...
    if (method === "GET" && url?.startsWith("/auth/callback")) {
      const parsedUrl = new URL(url, `http://localhost:${PORT}`);
      const code = parsedUrl.searchParams.get("code");

      if (!code) {
        return send(res, 400, { message: "Thiếu authorization code" });
      }

      const { cookies, user, message } = await handleOAuthCallback(code);
      res.setHeader("Set-Cookie", cookies);

      // Nếu có FRONTEND_URL → redirect về frontend
      const frontendUrl = process.env.FRONTEND_URL;
      if (frontendUrl) {
        res.writeHead(302, { Location: `${frontendUrl}/auth/success` });
        res.end();
        return;
      }

      // Không có frontend → trả JSON
      return send(res, 200, { message, user });
    }
